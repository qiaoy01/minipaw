...
use crate::channels::telegram::{
    ...
    TelegramAdmission, TelegramChannel, TelegramConfig, TelegramMessage,
};
use crate::config::{
...
impl App {
        ...
        match channel.admit_message(message) {
            TelegramAdmission::Accepted(text) => {
                ...
                let task_text = match kind {
                    MessageKind::Action => normalize_action_text(&text).to_owned(),
                    MessageKind::Knowledge => text.clone(),
                };
                println!(
                    "gateway route channel=telegram kind={} task={}",
                    match kind {
                        MessageKind::Action => "action",
                        MessageKind::Knowledge => "knowledge",
                    },
                    quote_compact(&task_text)
                );
                let outcome = self.agent.run_task(&task_text, self.llm.as_mut());
                let reply = match kind {
                    MessageKind::Action => telegram_action_reply(&outcome),
                    MessageKind::Knowledge => telegram_user_reply(&outcome.output),
                };
                print_gateway_event(
                    "session.message",
                    &session_key,
    ...
    fn handle_simulated_agent_message(
        ...
        let outcome = self.agent.run_task(&routed, self.llm.as_mut());
        print_gateway_event(
            "session.message",
            session_key,
            "agent",
